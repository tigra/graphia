...
| `make view-ledger` | Opens a standalone Textual viewer that reads the committed quality ledger ([`evals/blunder-ledger.yaml`](evals/README.md)) — the records the other targets write — as a scrollable table. Read-only, no model/AWS needed. `ARGS="--path <file>"` to point at an alternate ledger. |
