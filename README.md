...
- **Per-increment Tutorials** — [`/awos:tutorial`](.awos/commands/tutorial.md) → [`context/tutorials/`](context/tutorials/). A narrative learning artifact for each completed increment, teaching the concepts it introduced (depth-first, Socratic), with concept dedup against earlier tutorials so nothing is re-taught. Start with [Tutorial 001 — playable skeleton](context/tutorials/001-playable-skeleton/tutorial.md); [Tutorial 004](context/tutorials/004-robust-vote-input-validation/tutorial.md) is a good standalone read on a LangGraph interrupt/resume gotcha.
