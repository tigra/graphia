...
Both tests defensively stub ``fake_haiku`` *and* ``fake_sonnet`` so the
...
already kicked off the Sonnet-backed Mafia-pointing super-step. Without
the Sonnet stub, that call would hit ``ChatBedrockConverse`` with dummy
