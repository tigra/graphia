...
* **RNG** — ``GRAPHIA_SEED=0`` fixes role assignment and every tie-break.
  Seed 0 puts the human in slot 0 as Law-abiding, a well-trodden trajectory
  (see ``test_remote_mode_smoke.py``) that needs no ``kind="point"``
  interrupt answer.
...
async def test_local_and_remote_full_game_produce_identical_public_output(
    ...
    Drives one complete game in each mode with the same ``GRAPHIA_SEED``, the
