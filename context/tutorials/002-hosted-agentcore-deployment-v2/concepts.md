...
spec_title: Hosted AgentCore Deployment (interim — Slices 1–4)
introduced_on: 2026-05-13
---
...
- **Async-generator entrypoint streams super-steps as SSE** (`agentcore-async-generator-entrypoint`) — The `@app.entrypoint` handler is an `async def … -> AsyncIterator[dict]`; the SDK detects it via `inspect.isasyncgenfunction` and wraps each yielded dict as one SSE `data:` frame on the wire. Each LangGraph super-step yield arrives as one event on the client.
- **Start vs resume payload contract** (`runtime-payload-contract-start-resume`) — The Runtime's invocation surface accepts `{"action": "start", "thread_id", "initial_state"}` and `{"action": "resume", "thread_id", "resume_value"}`; same `thread_id` for start + every resume so the server-side `SqliteSaver` lands on the same checkpoint.
...
- **Nova direct on-demand in us-east-1** (`nova-direct-on-demand-us-east-1`) — Per ADR 003, Graphia invokes Amazon Nova Pro + Nova Lite as the gameplay + roster singletons directly against the foundation-model ARN in us-east-1; no inference profile, no cross-region routing. Retires the `regional-inference-profile-prefix` concept from spec 001's ledger.
...
## Crossing the wire (Slice 4)

- **boto3 invoke_agent_runtime is the client-side path** (`boto3-invoke-agent-runtime`) — The `bedrock_agentcore` SDK ships server-side primitives only; the client-side surface for invoking a deployed Runtime from outside the container is `boto3.client('bedrock-agentcore').invoke_agent_runtime`. The earlier-named `bedrock_agentcore.client.RuntimeClient` from the speculative tech-spec doesn't exist.
...
## Run-mode plumbing (wired up in Slice 4)
