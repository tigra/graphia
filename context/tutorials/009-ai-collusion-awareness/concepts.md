...
- **Paired-seed LLM A/B** (`paired-seed-llm-ab`) — A shared seed set reused across every condition pins the *game structure* (role deal, order) but not the LLM's words, so the residual is pure model noise and conditions can be compared as matched pairs.
