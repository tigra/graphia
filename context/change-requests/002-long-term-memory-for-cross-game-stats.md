...
| --------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- |
| `context/product/product-definition.md §1.4`                                | New success metric: cross-game stats accuracy and visibility; "advertised concepts" list widened to call out long-term cross-session AgentCore Memory; the CR-001 tool-use success metric was withdrawn and the day-chat readability metric softened to drop "tool-call beats" (cleanup applied 2026-05-07) | Yes |
| `context/product/product-definition.md §2.1`                                | New "Cross-game career and aggregate statistics" core feature; AgentCore deployment bullet widened to mention Memory's dual role; CR-001 "AI tool use during Day phase" and "Moderator tools" bullets were removed; the AgentCore Gateway tool-list narrowed to per-game diary read/write only; the Private diaries bullet softened from "queryable as a tool" to access-pattern wording (cleanup applied 2026-05-07) | Yes |
| `context/product/product-definition.md §2.2`                                | User journey now includes a pre-game career-summary greeting and a post-game career-stats panel; the CR-001 tool-call beat passage during Day phase was removed (cleanup applied 2026-05-07) | Yes |
| `context/product/product-definition.md §3.1`                                | Adds: cross-game stats capability, pre-/post-game stats display, long-term AgentCore Memory as the remote-mode store, local-file stats persistence for local mode; the CR-001 in-scope items covering AI-player and Moderator tool calls were removed; the Bedrock AgentCore Gateway in-scope item was narrowed to the per-game diary read/write surface (cleanup applied 2026-05-07) | Yes |
| `context/product/product-definition.md §3.2`                                | The "no cross-session persistence" item was tightened (only stats summaries persist); new out-of-scope for a dedicated `graphia stats` standalone command; new out-of-scope item added recording the AI tool-use deferral to Phase 7 with the design-driven-by-realistic-needs rationale (cleanup applied 2026-05-07) | Yes |
| `context/product/product-definition.md` — version                           | 1.1 → 1.2 (initial CR 002 application); 1.2 → 1.3 (after the tool-use cleanup, 2026-05-07)                                                                                                                                              | Yes |
...
- [x] Run `/awos:roadmap` to restructure the roadmap (Phase 2 AgentCore deployment, Phase 3 long-term Memory, Phase 7 tool-use demote). *Done 2026-05-06.*
- [x] Run `/awos:product` (Update Mode, §1.1, §1.2, §1.3, §1.4, §2.1, §2.2, §3.1, §3.2) to clean up the residual tool-use scope language: removed the AI Day-phase tools and Moderator-tools core features, softened the recap feature to read state directly, dropped the tool-use user-journey passage, removed the tool-use in-scope items, narrowed the Gateway item to the per-game diary surface, and added a new §3.2 item recording that tool-use is deferred to Phase 7. Version bumped 1.2 → 1.3. *Done 2026-05-07.*
