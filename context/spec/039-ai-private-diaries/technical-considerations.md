...
**Spec 038's highlighter must learn the tag too**, or a diary renders as undifferentiated body text: `eval_ledger` needs `KIND_DIARY`, a `_DIARY_TAG`, and entries in **`_MARKER_TAGS`** (is this line skeleton?) and **`_INLINE_CONTENT_KINDS`** (tag → body kind) — *corrected during Slice 4: the tech spec's `_TAG_KIND` names no symbol in the module; those two tables are what it meant.* Plus the attribute rule: **the owner `player` name is side-bearing** (ratified in Slice 4 — §2.8 prescribed the body and the `day` value but was silent here). It follows `_attr_kind`'s own rule, *the side colour lands on whichever token tells the reviewer the side — the role where a role is written, the name where none is*: a `<diary>` names a person and a Day but no role, exactly like `<thought>`. The achromatic alternative would render the same player's name two different ways a few lines apart in one file. One `elif` to reverse; no CSS depends on it, since `attr-mafia`/`attr-law-abiding` already exist. **`day` joins `_ATTR_NAMES`, which is a GLOBAL whitelist, not a diary-scoped rule** — any tag carrying `day="…"` will now split. Blast radius is nil today (zero occurrences across the 298 files, and the existing lookbehind stops `birthday="…"` false-splitting), but it is a global change, not a local one; `ui/ledger_viewer` needs a `transcript--diary` component class and CSS rule. **Note the palette is fully spent** — `$text-error` and `$text-primary` are the two sides, `$text-secondary` is `field-label`, `$text-muted` is `marker` and `thought`. A diary body should follow `thought`'s precedent (muted, and **not** side-tinted — a private reflection is not an act of allegiance) and distinguish itself on an SGR axis. **The real design problem, which this section originally failed to state:** a diary differs from body text easily; the hard part is differing from a **`thought`** — the one kind it shares a colour and a register with. **And the SGR axis is a choice of ONE, not an open choice** (settled in Slice 4): `bold` is the reviewer's seat, `reverse` is `recap`'s landmark, `italic` must be *kept* because it is what makes a diary read as thought's sibling, `dim`/`strike`/`blink` are out on legibility and meaning — and **`overline` is dead in this Textual**: it parses (`o` is in `VALID_STYLE_FLAGS`) and reaches the resolved Rich style, but `textual.style.Style.from_rich_style` has **no `overline` parameter**, so `Content.from_rich_text` drops it and the rule paints nothing. That leaves **`underline`**: `thought`'s rule plus one axis. **Also structurally unavailable, not merely spent: `background:`** — `_kind_styles` strips it with `without_color` and adds back only the foreground, so a filled-block kind cannot be expressed here at all. **And do not reach for length as the differentia** — measured across the corpus a `<thought>` body runs **46–626 characters** (n=8,183, median 188), overlapping a diary at the short end, so mass reinforces but the per-row rule is the mechanism; the `day` attribute is an `attr` span.
...
**A third instance of the same family — but the mechanism I gave was wrong, corrected in Slice 5.** `_meta_get` returns `str(value)`, so a raw `False` renders as `diaries=False` rather than being dropped; only `None` and `""` are omitted (verified: `_meta_get({'d': False}, 'd') == 'False'`). **Pass the string `"on"` / `"off"` anyway** — the header is CLI vocabulary and should not carry a Python repr — but not for the reason originally stated.
...
### 2.12 Three things Slice 5 established that the spec had left ambiguous

- **The sentinel lives in `main()`, not `run_eval`.** "Fail fast before game 1" was ambiguous between the two, and the choice has a real cost: a hard raise inside `run_eval` breaks ~15–18 pre-existing tests across four files whose `_Cfg` stubs document that `run_eval` reads config **defensively**. The consequence to record: a **direct** `run_eval(...)` call with a config that cannot answer writes an *unlabelled* record rather than refusing — honest (blank, never a wrong label), but the refusal is a **CLI-layer** guarantee, not a writer-layer one.
...
### 2.15 Three §2.10 clarifications recorded in Slice 5's render task

- **"All 30 render byte-identically" and "close the `scripted_player` gap" cannot both hold** — closing the gap necessarily adds a line to the 10 records that carry that field. The provable claim, which is what the spec should have asked for: **the diaries renders add nothing to any committed record** (0 of 30 carry the arm or any `diary_*` key), **and the `scripted_player` render adds exactly one line to exactly the records carrying the key.** Verified: filtering that one line out of both sides yields an empty diff across all 30.
- **The table says `on`/`off` while the drill-down says `True`/`False`, deliberately.** §2.10 authorised a `Diaries` column without saying which spelling. The split is the right one and is now on the record: a drill-down line shows the record's **actual value**, like every other settings line, so cell text and on-disk value cannot drift (`_kind_cell`'s verbatim rule); a table cell is **read at a glance**, which is the column's entire justification, and `Stand-in` already sets the precedent for a label-style cell. A reviewer seeing both should know they are one field, not two.
