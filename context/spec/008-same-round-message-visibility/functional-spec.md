...
- **As a** player taking my turn, **I want** to see what others said earlier this round, **so that** I can respond to the live discussion.
  - **Acceptance Criteria:**
    - [x] When an AI player takes its speaking turn, the messages other players submitted earlier in the same round are included in what the AI is given to respond to.
    - [x] When it is the human's turn, the messages submitted earlier in the same round are visible in the on-screen discussion before the human speaks.
    - [x] A message from an earlier speaker this round is present for a later speaker in the same round (verified at a representative later position, for both an AI and the human).
...
- **As a** player, **I want** the shown discussion to be large enough that I don't miss earlier speakers from this round.
  - **Acceptance Criteria:**
    - [x] The amount of recent discussion shown to a speaker is the **last 30 messages** (increased from the previous smaller amount).
    - [x] For a normal-sized game, this is enough to include **every** earlier speaker from the current round (a full round of speeches plus the day's opening announcement fits within the shown discussion).
    - [x] Messages older than the shown extent (e.g. from several rounds earlier) are **not guaranteed** to be visible — this is an acknowledged limit, not a defect.
...
- **As the** project maintainer, **I want** this locked in by tests in the standard suite, **so that** a future change can't silently stop showing players the round's discussion.
  - **Acceptance Criteria:**
    - [x] A test drives a Day round and confirms a later AI speaker is given an earlier same-round speaker's message.
    - [x] A test confirms the human's on-screen discussion shows earlier same-round messages before the human's turn.
    - [x] A test confirms the shown extent covers a full round for the standard lineup.
    - [x] The tests run as part of the normal test command and reach no external or cloud service.
...
- **As a** player, **I want** the Day to behave as before apart from seeing more of the discussion.
  - **Acceptance Criteria:**
    - [x] Apart from showing more recent discussion, the Day plays as today — same turn flow, prompts, number of rounds, vote rules.
    - [x] No new on-screen control or setting is introduced.
