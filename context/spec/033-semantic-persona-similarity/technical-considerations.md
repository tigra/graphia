...
- **New pure-ish fn `score_persona_semantic_sim(players, embed_fn) -> {"mean": float | None, "peak": float | None, "denominator": int}`** — `embed_fn` is **injected** (so tests pass a deterministic fake; production passes `get_embeddings().embed_documents`).
  ...
  - `embed_fn(texts)` → one vector per persona (**single batch call per game**). Cosine of each unordered pair; `mean` = average cosine and `peak` = max cosine over `C(n,2)` pairs; `denominator = C(n,2)`. `<2` AI personas → `{None, None, 0}`.
...
- Per game, call `score_persona_semantic_sim(cap.players, embed_fn)` and accumulate the **sum of cosines** + a **running max** + pair count across games; after the loop, when pairs > 0, record two value-type facets (no CI — reuses 032's `_attach_ci` skip-when-no-`count`): `result.metrics["persona_sem_mean"] = {"mean": cos_sum_total / pairs_total, "denominator": pairs_total}` and `result.metrics["persona_sem_peak"] = {"peak": cos_max_run, "denominator": pairs_total}`.
...
- Append `("persona_sem_mean", "persona sem mean")` and `("persona_sem_peak", "persona sem peak")` to `METRIC_ORDER` (after the 032 lexical entries). **No new rendering code** — both reuse 032's value-type `~{value:.2f} (n=…)` branch.
...
- **Validity is not assumed (effort-not-results, CR 005).** Out-of-suite: run a measured batch, compute `persona_sem_mean` and the lexical `persona_lex_mean` on the same runs, and record whether the semantic measure reads **higher on archetype-samey casts** than the lexical one (confirmed or refuted — both complete).
...
- **Record + viewer:** mocked `run_eval` writes `persona_sem_mean {mean, denominator}`; reuses 032's value-type render (`~mean (n=…)`); absent → blank; no CI; `METRICS_VERSION` unchanged. Reuse the ledger/transcript redirect.
