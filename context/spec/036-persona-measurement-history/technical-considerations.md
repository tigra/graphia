...
The four persona metrics need **no new plumbing at all**: `persona_lex_mean`, `persona_lex_peak`, `persona_sem_mean` and `persona_sem_peak` are already in `METRIC_ORDER` (specs 032/033) and already render through the value-type branch. The viewer gains exactly one new column.
...
- **`render_detail`** needs no change: it iterates the blocks present, so `generation` and `run.kind` appear automatically.
