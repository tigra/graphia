...
- [x] **A/B to isolate the cause** — the quick n=2 A/B was too noisy to trust (a same-config replication swung 33%→47% near-dup). Replaced it with a **rigorous experiment** ([`repetition-experiment.md`](./repetition-experiment-design.md), `make repetition-experiment`, `src/graphia/tools/repetition_experiment.py`): 9 conditions × N=10 paired games, length-capped, name-masked metric, bootstrap CIs, paired-vs-HEAD with Holm. Result: every lever helps, but `noline` alone barely does (Δ−0.13) — you must *instruct against* parroting, not just stop priming it. **[Agent: testing]**
- [x] **Decide + apply the fix** — adopted **`antiparrot`**: reword the `DAY_SPEAK_SYSTEM` line to "Say something new on your turn — don't repeat or echo a point another player has already made." Measured **0.15 [.09,.20]** name-masked near-dup vs HEAD's **0.57** (Δ−0.42, Holm p<.001) — *below* the pre-spec baseline (0.20) — while keeping 008's window (30) and temp (0.7). No change-request: revised in place since 009 was still being iterated. Spec/tech updated to the anti-parroting framing.
