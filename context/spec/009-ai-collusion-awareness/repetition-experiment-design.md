...
- **Paired design (variance reduction, *not* exact matching).** A fixed seed set `S = {seed_base + i | i in 0..N-1}` is reused **identically across all conditions**, so the RNG *stream* and the opening **role deal** are identical across conditions and the scripted human is fixed. That is the limit of the guarantee. The seed does **not** match the trajectory: the model decides votes and the Night victim, so *who is alive* (hence speaking order and game length) diverges between conditions; and because the single RNG stream is consumed at LLM-decided points (a tie-break only draws *when the picks tie*), an LLM-contingent draw in one condition desynchronises the stream, so even downstream *mechanical* outcomes differ. Pairing therefore only *correlates* the paired runs — removing the variance from the shared opening and sharpening the paired test — without making the two games the same. Residual variance is averaged over N. (This weakens the precision of the pairing, not the *validity* of the paired test, which holds for any correlated pairs.)
