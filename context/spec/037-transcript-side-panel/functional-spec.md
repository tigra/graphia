...
- **Status:** Completed *(verified 2026-08-31 — all 22 acceptance criteria met; suite green at 1289 passed / 1 skipped. One criterion was met but UNTESTED at verification — a games list longer than the panel is tall — and a test was added rather than the criterion being taken on trust; four claims in the tech spec proved wrong against the installed Textual and were corrected during implementation)*
...
- **A run's games are listed in a narrow panel beside its details, always visible.**
  ...
  - **Acceptance Criteria:**
    - [x] Given a run with preserved games, when the reviewer opens that run, then its games are listed in a narrow panel on the right and the run's figures are shown to the left of it, both visible at once.
    - [x] Given the reviewer is looking at a run, when they have pressed nothing since opening it, then the panel is already there — it is not something they have to summon.
    - [x] Given a run with many games, when the list is longer than the panel is tall, then the list can be moved through to reach every game.
...
- **One keypress moves between the figures and the list, in both directions.**
  ...
  - **Acceptance Criteria:**
    - [x] Given focus is on the run's figures, when the reviewer presses right, then focus moves to the games list and it is visibly the active area.
    - [x] Given focus is on the games list, when the reviewer presses left, then focus returns to the run's figures and they are visibly the active area.
    - [x] Given focus is already on the games list, when the reviewer presses right, then nothing happens — focus stays in the list and the screen does not change.
    - [x] Given focus is already on the run's figures, when the reviewer presses left, then nothing happens — focus stays on the figures and the screen does not change.
...
- **Up and down act on whichever area has focus.**
  ...
  - **Acceptance Criteria:**
    - [x] Given focus is on the games list, when the reviewer presses down then up, then the highlighted game moves down one and back up one, and the figures do not scroll.
    - [x] Given focus is on the run's figures and they are taller than the screen, when the reviewer presses down then up, then the figures scroll down and back up, and the highlighted game does not change.
...
- **The dedicated transcripts key still works, as a shortcut into the panel.**
  ...
  - **Acceptance Criteria:**
    - [x] Given focus is on the run's figures, when the reviewer presses the transcripts key, then focus moves into the games list and no new screen opens.
    - [x] Given the reviewer presses the transcripts key, when they look at the screen, then the run's figures are still visible beside the list — nothing has been covered up.
...
- **Opening a game shows it full-screen, and coming back restores the reviewer's place.**
  ...
  - **Acceptance Criteria:**
    - [x] Given focus is on the games list with a game highlighted, when the reviewer chooses it, then that game's full text fills the screen.
    - [x] Given the reviewer is reading a game's full text, when they go back, then they return to the run's figures with the games panel visible and **the same game still highlighted**.
    - [x] Given the reviewer returns from a game and moves down the list and opens the next game, then that next game's text is shown — moving between games takes two keypresses, with no menu in between.
...
- **The intermediate games-list screen is gone.**
  ...
  - **Acceptance Criteria:**
    - [x] Given the reviewer is anywhere in the viewer, when they navigate by any available key, then they never arrive at a screen whose only content is a list of a run's games.
    - [x] Given the reviewer opens a game and goes back, then they land on the run's figures — not on a list screen they then have to leave as well.
...
- **Runs with no preserved games keep the panel, with a short explanation.**
  ...
  - **Acceptance Criteria:**
    - [x] Given a run with no preserved games, when the reviewer opens it, then the panel is present in the same place and shows a short note that this run has no preserved games.
    - [x] Given a run with no preserved games, when the reviewer moves focus into the panel and presses up or down, then nothing happens and no error appears.
    - [x] Given the reviewer moves between a run with games and a run without, then the width of the figures area stays the same — the layout does not jump.
...
- **Everything else about the viewer is unchanged.**
  ...
  - **Acceptance Criteria:**
    - [x] Given the reviewer is on the list of runs, when they look at it, then it shows the same runs and the same columns as before this change.
    - [x] Given the reviewer opens a game's full text, when they read and scroll it, then it looks and behaves exactly as it did before this change.
    - [x] Given a reviewer used the viewer before this change, when they use it after, then the only difference they encounter is that a run's games are beside the figures rather than behind a menu.
