...
- **Moderator memory of past games (deferred future direction — likely a future change request):** persisting each game's end-of-game Moderator recap and letting the Moderator *occasionally recall similar past games* during play — with each game given a unique (or near-unique) **city name** so the player can tell which past game is being referenced. This is a richer use of long-term memory (narrative recall) layered on top of this spec's exact-counter career stats; it touches the Moderator recap (a Phase 6 concern) and introduces per-game city-name identity, so it is intentionally **not** built here. This spec only persists the exact career-stat numbers, not recaps.
