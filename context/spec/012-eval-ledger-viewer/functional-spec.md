...
- **As the** maintainer, **I want** to scroll the table in both directions, **so that** a long history or a wide set of columns is fully reachable without truncation.
  - **Acceptance Criteria:**
    ...
    - [ ] Given more columns than fit on screen, when the maintainer scrolls **left/right**, then the off-screen columns come into view, the headers moving in step with their columns.
...
- **As the** maintainer, **I want** to open any run's complete record, **so that** I can read everything the row can't fit — full provenance, exact counts, and the note.
  - **Acceptance Criteria:**
    ...
    - [ ] When the maintainer **leaves** the full-record view, then they **return to the table** in the same place they left it.
...
- A **terminal table view** of the eval ledger: one row per recorded run, labelled column headers, the run's identifying facts plus one column per watched behavior (each cell showing rate + confidence interval + sample, when present; empty when the behavior wasn't exercised), and a visible mark for runs recorded from a modified working copy.
- **Vertical and horizontal scrolling** of a table larger than the screen, with headers that stay put vertically and track their columns horizontally.
- **Search that filters** the rows to those matching a typed query, with a match-count indicator and a clear "no matches" state.
