...
1. **`ChatOllama.with_structured_output(..., method="json_schema")` carried its own verify-at-implementation gate, and that gate is now satisfied** — measured 2026-09-02, the same discipline ADR 010 applied and the reason this defect was caught by a plan rather than by surprise. Findings: all **seven** schemas bind under `method="json_schema"`; the diary prompts returned **18/18** schema-valid (against 11/24 failures on the tool-use path); and the routing leaves an assertable wire-level trace (`format == Schema.model_json_schema()`, no `tools`), so it can be pinned by test rather than trusted. Two things the measurement corrected in this document's first draft: the count is **seven**, not six (`Persona` was omitted below), and `method="json_schema"` turns out to be `ChatOllama`'s **own default** — which reduces the seam from a correctness requirement to a defensive one, since `ChatBedrockConverse` defaults to `function_calling` and must not receive the Ollama method. The live smoke gate over all seven, in one scripted game, remains the acceptance condition (spec 041 §2.7).
2. **The application-layer prose recovery that shipped as the interim repair** (Alternative 1, commit `b9bfbd4`) is **withdrawn** when the reliable route lands. It did its job — it unblocked measurement immediately — but it is a *compensation*, and keeping it beneath an enforcing mechanism leaves two paths that can produce an entry with no way to tell from the outside which one did. Reliable decoding **replaces** it rather than layering on top of it, so exactly one mechanism is responsible.

   _Revision note (2026-09-02): this ADR was first written saying the recovery would be **kept** as a provider-agnostic backstop. That was the author of this document scoping beyond what was ratified, and it was corrected the same day when the decision was taken forward into spec 041 — "instead of fallback, reliable decoding". The original reasoning (it costs nothing once written, and covers providers this ADR does not) is recorded here because it is the argument that would have to be answered if a future provider needs a backstop again._
...
- **Recovery could never have served a multi-field schema, which is why it is not kept.** The interim repair worked only because `Diary` has one free-text field. `Roster`, `Persona`, `Pointing`, `Ballot` and `DayAction` have no readable form outside the shape requested — a vote or a pointing choice cannot be reconstructed from prose — so a backstop was never a general answer, only a local one. (`Reflection` is the one other single-field schema, and it never failed: 0 of 391 on the same model in the same runs.) Withdrawing it removes the temptation to treat it as coverage it never had.
- **The grammar constrains SHAPE, not CONTENT — so "non-conforming output is unrepresentable" is true only of the JSON shape.** Measured: `DayAction.model_json_schema()["required"] == ["kind"]`, because its mutual-exclusion rule lives in a Pydantic `@model_validator` that JSON Schema cannot express — so `{"kind": "vote"}` is grammar-valid and model-invalid. `Roster`'s case-insensitive distinctness is validator-only too, and `Pointing` can return a well-formed id naming nobody. **Three of seven kinds therefore keep a live substitution path after this decision**, reached through content rejection rather than unreadable output. With a realistic prompt `DayAction` measured 0/6 failures on both the old and new paths, so this is pre-existing rather than introduced — but it bounds what the decision can promise, and anyone reading the enforcement claim above needs it.
- **After withdrawal there is no second line of defence, and that is the accepted trade.** If native `format` regresses or is dropped, failures become immediate and total on this path rather than partially absorbed. This is deliberate: a partially-absorbed failure is exactly what hid the original defect for weeks. The mitigation is the per-answer-kind substitution rate recorded in every measured run, plus the visible marking in preserved transcripts, so a regression is loud rather than silent.
...
- **Related CRs:** _none._
- **Related specs:** `context/spec/039-ai-private-diaries/` — the feature that exposed the defect, including `DIARY_SYSTEM` in `src/graphia/prompts.py` and the disqualified measured arm. `context/spec/041-reliable-structured-decoding/` — the specification that carries this decision into implementation, and whose scope withdrew the interim recovery per the revision note in §3.
