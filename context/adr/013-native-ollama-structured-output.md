...
- **Recovery could never have served a multi-field schema, which is why it is not kept.** The interim repair worked only because `Diary` has one free-text field. `Roster`, `Pointing`, `Ballot` and `DayAction` have no readable form outside the shape requested — a vote or a pointing choice cannot be reconstructed from prose — so a backstop was never a general answer, only a local one. Withdrawing it removes the temptation to treat it as coverage it never had.
