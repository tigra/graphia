...
- [x] **Setup Flexibility**
  - [x] **Configurable Role Counts:** Replace the fixed lineup with the way for a human to specify numbers of Law-abiding Citizens and Mafiosos, then randomly assign roles so every run starts fresh. *(Spec 014, Completed 2026-06-16. The **default** these counts fall back to is **6 Law-abiding + 2 Mafiosos** per [CR 007](../change-requests/007-starter-lineup-balance-claim-and-default.md), agreed 2026-09-03; it was 5-and-2 before that. Town win rates are comparable only **within** one lineup — and that is a lineup distinction, not a date one: four records from 2026-06-21/22 were already played at 6-and-2, so the field on each record decides comparability, never when it was written. Note also that 6 Law-abiding + 2 Mafiosos is **eight players including the human**. Going further than 6 is not simply better: 7-and-2 measures worse than 6-and-2, because the margin widens only with every second citizen while the odds of any execution finding a Mafioso keep falling.)*
