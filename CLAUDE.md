...
- **Dialogue-quality evals (real Bedrock, NOT in `pytest`):** `make eval-dialogue` plays N real-Nova games with a scripted human and scores AI Day-speech repetition (lexical near-dup); `make repetition-experiment` runs the rigorous paired A/B that ranks prompt/window/temperature fixes with bootstrap CIs (design + results: `context/spec/009-ai-collusion-awareness/repetition-experiment-design.md`). These hit live Bedrock and cost tokens, so they live behind make targets, not the mocked suite. Use them to A/B any change to AI dialogue.
